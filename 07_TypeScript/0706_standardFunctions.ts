{
  ...
  const isAlphanumeric = "123aX".search(/\W/) === -1
  if (isAlphanumeric) {
    console.log("123aX は英数字のみ")
  }
  const isNotAlphanumeric = "123@aX".search(/\W/) !== -1
  ...
  let textArray = ["apple", "orange", "grape"]
  console.log("textArray: " + textArray)

  // 要素の検索
  console.log('textArray.indexOf("orange"): ' + textArray.indexOf("orange"))

  // 位置を指定して配列から取り除く
  // 上記indexOfと併用して任意の要素を取り除くことができる
  textArray.splice(textArray.indexOf("orange"), 1)
  console.log('textArray.splice(textArray.indexOf("orange"), 1)')
  console.log("textArray: " + textArray)

  // 配列のコピー textArray.concat()でも可能
  const copyArray = textArray.slice()
  textArray = []
  console.log("copyArray = textArray.splice()")
  ...
  console.log("new Date(2020, 01, 02, 03, 04, 65): " + new Date(2020, 1, 2, 3, 4, 65))
  ...
  // 整数で表現できる最大値 使用するにはオプションの指定が必要
  // console.log("Number.MAX_SAFE_INTEGER: " + Number.MAX_SAFE_INTEGER)
  // 整数で表現できる最小値 使用するにはオプションの指定が必要
  ...
  // 文字列から整数を返す 何進数で変換するか指定しないとエラー
  console.log('parseInt("4"): ' + parseInt("4", 10))
  console.log('parseInt("4.25"): ' + parseInt("4.25", 10))
  console.log('parseInt("5@"): ' + parseInt("5@", 10))
  console.log('parseInt("h4fg"): ' + parseInt("h4fg", 10))
